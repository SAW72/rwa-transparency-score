...
| Backed / xStocks **redemption** | Public markdown: [issuance and redemption](https://docs.xstocks.fi/docs/issuance-and-redemption.md) (+ optional [xPort in-kind](https://docs.xstocks.fi/docs/issuance-and-redemption/in-kind-flow-xport.md)) | Live hook in `BackedVerifier.verify_redemption`. Score **80** when primary-market redemption language is present (KYC/whitelist called out); **85** only if the in-kind page actually says tokens redeem back into shares. Marketing Webflow shells are **not** used (JS-only — no silent claim). Fetch/parse failure → labeled **heuristic fallback**. |
| Dinari **redemption** | Public markdown: [What are dShares](https://docs.dinari.com/docs/what-is-dshare.md) | Live hook in `DinariVerifier.verify_redemption`. Score **80** when the page documents mint/burn as issuance/redemption (brokerage fill, funds transfer). Not treated as in-kind share delivery. Fetch/parse failure → labeled **heuristic fallback**. |
...
**Re-check 2026-09-17 (A2 / #23):** still **no aggregator `proxyAddress`** for the xStocks line. The [SmartData directory](https://docs.chain.link/data-feeds/smartdata/addresses) now lists `TSLAx` / `AAPLx` / `NVDAx` / … **DataLink PoR streams** with `proxyAddress: null` (pull-based Data Streams, not `latestRoundData`). Reference-data JSON catalogs (Polygon / Ethereum / Solana / Avalanche / BSC) still only publish Backed **bToken** PoR proxies. xStocks REST PoR (`GET /public/proof-of-reserves/{symbol}`) is issuer JSON — not wired as on-chain PoR. Full notes: [`docs/XSTOCKS_CHAINLINK_POR.md`](docs/XSTOCKS_CHAINLINK_POR.md). When a non-null SmartData proxy ships, add it to `XSTOCKS_POR_FEEDS` — do not invent addresses.
...
Each selected compare slot has a **Scorecard** expander (chevron sits next to the label). It does **not** run on page load. Click **Scorecard** to build and preview a signed, timestamped PNG (ticker, score, band, all six pillar bars including **basis**, RAT branding). That click does **not** post to X — no `attach_x_share` / `post_image` / `XClient`. After the card is visible, **download PNG** and a **Share** button at the **bottom** of that view are separate explicit actions — not an auto-flow. The Share button stays **Share** (never renamed to “posted to X”). Preview/download use **markdown data URIs** (not `st.image`, `st.download_button`, or `components.html`). Those widgets register `/media`, `/_stcore/download`, or a `/component` iframe that Streamlit 1.39 MPA v1 reports as **Page not found**. There is no `pages/` directory, so the app stays a single-page script and that router cannot fire on Share. Only **Share** calls the X API (simple `POST /2/media/upload` for the PNG, then `POST /2/tweets`; chunked initialize/append/finalize is only a fallback — never `command=INIT`). If credentials are missing, or the X post cannot be published, the PNG still downloads and the UI shows a polished skip message — never raw X API / HTTP errors. Missing credentials and an X API upload/tweet failure use **different** skip copy so the live banner is not ambiguous. Operator-only `X_SHARE_FAIL` lines go to stdout / Render logs (HTTP status + short redacted body); they are never shown in the UI. The share footer is a clickable **Open score page** markdown link (and **View post on X** only when a real status URL came back) — never a raw HTTPS dump, and never an invented tweet URL. An empty/failed PNG shows an error plus retry, not a dead button. Generation or X failures never crash the demo. PNG generation and tweet caption are unchanged.
...
Set them in the Render dashboard (`sync: false`) or Bitwarden. Never commit them. Posting uses the app account — click **Share** at the bottom of the Scorecard view only when you intend to publish. Opening Scorecard is not a post. This is a user-triggered share, not a hunter or auto-poster.
