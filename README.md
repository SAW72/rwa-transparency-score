...
`POST /v1/webhooks` with `{"url": "https://…", "trigger": "band_cross"}` or `"below_orange"` (new band is RED).
...
`GET /v1/attest/{ticker}` returns `score_hash` (SHA-256 of the canonical breakdown). Submit that hash with Foundry — see [`contracts/README.md`](contracts/README.md). Deploy scripts **revert on any chain except Base Sepolia (84532)**. Mainnet is held.
